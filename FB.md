...
클럭 신호 = 0,1 이 주기적으로 나오는 신호다.[] 요소 : 크기, 주파수, 듀티

PWM - pluse width modulation = 펄스의 폭을 컨트롤하기 위한 주기 제어 방법이다.
- 출력되는 전압값을 일정한 비율 이상은 1,  나머지는 0 으로 처리 = 여기서의 비율을 듀티라고 함
- 소형모터 0~12 볼트 => 이는 모터의 회전 속도를 결정, => 펄스 폭 제어가 모터의 속도를 결정하게됨.
예를들어 0 ~ 10볼트가 있고, 1볼트는 펄스폭을 10 퍼센트로 만들어 = 이는 듀티사이클이 10%을 의미.

signal tmp: integer : range 0 to 100;
